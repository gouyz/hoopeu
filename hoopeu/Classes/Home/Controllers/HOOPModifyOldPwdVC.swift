...
class HOOPModifyOldPwdVC: GYZBaseVC {
    ...
    @objc func clickedNextBtn(){
        if pwdTxtFiled.text!.isEmpty {
            MBProgressHUD.showAutoDismissHUD(message: "请输入旧密码")
            return
        }
        requestCheckPwd()
    }
    /// 检测旧密码
    func requestCheckPwd(){
        ...
        GYZNetWork.requestNetwork("user/verifyPassword", parameters: ["password":pwdTxtFiled.text!],  success: { (response) in
            ...
            if response["code"].intValue == kQuestSuccessTag{//请求成功
                ...
                if response["data"].boolValue{
                    ...
                    MBProgressHUD.showAutoDismissHUD(message: "旧密码不正确")
                }
