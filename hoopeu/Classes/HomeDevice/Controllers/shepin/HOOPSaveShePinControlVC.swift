...
class HOOPSaveShePinControlVC: GYZBaseVC {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "射频遥控器命名"
    ...
    @objc func clickedSendBtn(){
        ...
        if isEdit {
            paramDic["msg_type"] = "app_pt2262_edit"
        }else{
            paramDic["msg_type"] = "app_pt2262_add"
        }
    ...
    override func mqtt(_ mqtt: CocoaMQTT, didReceiveMessage message: CocoaMQTTMessage, id: UInt16 ) {
        ...
        if let data = message.string {
            ...
            if type == "app_pt2262_add_re" && phone == userDefaults.string(forKey: "phone"){
                //                hud?.hide(animated: true)
                MBProgressHUD.showAutoDismissHUD(message: result["msg"].stringValue)
                if result["code"].intValue == kQuestSuccessTag{
                    let _ = navigationController?.popToRootViewController(animated: true)
                }
            }else if type == "app_pt2262_edit_re" && phone == userDefaults.string(forKey: "phone"){
                //                hud?.hide(animated: true)
                MBProgressHUD.showAutoDismissHUD(message: result["msg"].stringValue)
                if result["code"].intValue == kQuestSuccessTag{
                    let _ = navigationController?.popToRootViewController(animated: true)
