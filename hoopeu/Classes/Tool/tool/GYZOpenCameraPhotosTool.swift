    ...
    func openPhotoLibrary(controller : UIViewController,  editor : Bool){
        ...
        checkPhotoPermission { (granted) in
            if granted{
                ...
                weakSelf?.goPhotoLibrary(controller: controller,editor :editor)
            }else{
                weakSelf?.showPermissionAlert(content: "请在iPhone的“设置-隐私”选项中，允许访问你的手机相册",controller : controller)
            }
        }
        
    }
    
    func goPhotoLibrary(controller : UIViewController,  editor : Bool){
