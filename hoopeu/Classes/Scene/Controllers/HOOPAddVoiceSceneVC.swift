...
private let addVoiceSceneCell = "addVoiceSceneCell"
private let addVoiceSceneHeader = "addVoiceSceneHeader"

class HOOPAddVoiceSceneVC: GYZBaseVC {
    ...
    lazy var tableView : UITableView = {
        ...
        table.register(HOOPAddVoiceSceneCell.self, forCellReuseIdentifier: addVoiceSceneCell)
        table.register(LHSGeneralHeaderView.self, forHeaderFooterViewReuseIdentifier: addVoiceSceneHeader)
    ...
    func requestSceneInfo(){
        ...
        GYZNetWork.requestNetwork("scene/voice", parameters: ["id":sceneId],method : .get,  success: { (response) in
    ...
    func setSceneInfo(){
        if sceneDetailModel != nil {
            ...
            for item in (sceneDetailModel?.sceneSayList)!{
                sayArr.insert(item.content!, at: 0)
            }
    ...
    /// 删除
    func deleteScene(){
        let paramDic:[String:Any] = ["token":userDefaults.string(forKey: "token") ?? "","phone":userDefaults.string(forKey: "phone") ?? "","scene_id":Int.init(sceneId)!,"msg_type":"app_scene_del","app_interface_tag":""]
    ...
    @objc func clickedSaveBtn(){
        ...
        if sayArr.count == 1 {
            MBProgressHUD.showAutoDismissHUD(message: "请添加相似说法")
            return
        }
        if doDicArr.count == 0 {
        ...
        createHUD(message: "加载中...")
        var says = sayArr
        says.remove(at: sayArr.count - 1)
        var paramDic:[String:Any] = ["token":userDefaults.string(forKey: "token") ?? "","phone":userDefaults.string(forKey: "phone") ?? "","scene_name":sceneName,"says":says,"cmds":doDicArr,"app_interface_tag":""]
        if isEdit {
            paramDic["scene_id"] = Int.init(sceneId)!
            paramDic["msg_type"] = "app_scene_edit"
        }else{
            paramDic["msg_type"] = "app_scene_add"
        }
    ...
    /// 编辑对叮当宝贝说
    func goSceneSayVC(index: Int){
        let vc = HOOPEditSayVC()
        if index != sayArr.count - 1 {//修改已有相似说法
            vc.sayContent = sayArr[index]
            vc.isEdit = true
        }
        vc.resultBlock = {[weak self](name) in
            
            if index == (self?.sayArr.count)! - 1 {//添加相似说法
                self?.sayArr.insert(name, at: index)
            }else{
                if name == "" {// 删除
                    self?.sayArr.remove(at: index)
                }else{
                    self?.sayArr[index] = name
                }
            }
            self?.tableView.reloadData()
        }
    ...
    override func mqtt(_ mqtt: CocoaMQTT, didReceiveMessage message: CocoaMQTTMessage, id: UInt16 ) {
        ...
        if let data = message.string {
            ...
            if ((type == "app_scene_add_re" && !isEdit) || (type == "app_scene_edit_re" && isEdit) || type == "app_scene_del_re") && phone == userDefaults.string(forKey: "phone"){
...
extension HOOPAddVoiceSceneVC: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: addVoiceSceneCell) as! HOOPAddVoiceSceneCell
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headerView = tableView.dequeueReusableHeaderFooterView(withIdentifier: addVoiceSceneHeader) as! LHSGeneralHeaderView
        
        if section == 0 {
            headerView.nameLab.text = "场景名称"
        }else if section == 1{
            headerView.nameLab.text = "对叮当宝贝说"
        }else{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 0 {// 编辑场景名称
            goSceneNameVC()
        }else if indexPath.section == 1{//对叮当宝贝说
            goSceneSayVC(index: indexPath.row)
        }else{//叮当宝贝会做
